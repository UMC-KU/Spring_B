...
import static com.example.demo.config.BaseResponseStatus.GET_DAO_POST_FAIL_ERROR;
...
public class PostDao {
    ...
    public List<GetPostsRes> selectPosts(int userIdx){
        String selectUserPostsQuery = "\n" +
                ...
                "            left join Follow as f on f.followedIdx = p.userIdx and f.status = 'ACTIVE'\n" +
                "            left join PostLike as pl on pl.userIdx = f.followingIdx and pl.postIdx = p.postIdx\n" +
                "        WHERE f.followingIdx = ? and p.status = 'ACTIVE'\n" +
                "        group by p.postIdx;\n" ;
    ...
    public int createPost(int userIdx, String content){
        String createPostQuery = "insert into Post(userIdx, content) VALUES (?,?)";
        Object[] createPostParams = new Object[] {userIdx, content};
        this.jdbcTemplate.update(createPostQuery,
                createPostParams);
    ...
    public int insertPostImgUrls(int postIdx, PostImgUrlsReq postImgUrlReq){
        String createPostImgUrlQuery = "insert into PostImgUrl(postIdx, imgUrl) VALUES (?,?)";
        Object[] createPostImgUrlParams = new Object[]{postIdx, postImgUrlReq.getImgUrl()};
        this.jdbcTemplate.update(createPostImgUrlQuery,
                createPostImgUrlParams);

        String lastInsertIdxQuery = "select last_insert_id()";
        return this.jdbcTemplate.queryForObject(lastInsertIdxQuery,int.class);
    }

    public int modifyPost(int postIdx, String content){
        String modifyPostQuery = "update Post set content = ? where postIdx = ? ";
        Object[] modifyPostParams = new Object[]{content, postIdx};
